...
class Controller {
  ...
  static getAllTask(req, res, next) {
    Task.findAll()
      .then((data) => {
        res.status(200).json({ message: 'sukses menampilkan data', data });
      })
      .catch(next);
  }
  ...
  static getOneTask(req, res, next) {
    Task.findOne({
      ...
      .then((data) => {
        res.status(200).json({ message: 'sukses menampilkan satu data', data });
      })
      .catch(next);
  }
  ...
  static update(req, res, next) {
    ...
    Task.update(
      ...
      .then((data) => {
        res.status(200).json({ message: 'Berhasil update data', data });
      })
      .catch(next);
  }
